...
/**
   ...
   */
const initScrollSpy=()=>{if(document.querySelector(".section.main")){let editToggle=document.querySelector("form.editmode-switch-form");if(!editToggle)return;if(editToggle.addEventListener("click",(()=>{window.sessionStorage.setItem("theme_boost_union_scrollspy_edittoggled",!0);let viewporttop=document.getElementById("page").scrollTop,closest=null,closestoffset=null;document.querySelectorAll(".section.main").forEach((node=>{let thisoffset=node.offsetTop;closest&&closest.offsetTop&&(closestoffset=closest.offsetTop),(null===closest||Math.abs(thisoffset-viewporttop)<Math.abs(closestoffset-viewporttop))&&(closest=node)})),window.sessionStorage.setItem("theme_boost_union_scrollspy_closestid",closest.id),window.sessionStorage.setItem("theme_boost_union_scrollspy_closestdelta",viewporttop-closest.offsetTop)})),window.sessionStorage.getItem("theme_boost_union_scrollspy_edittoggled")){let closestid=window.sessionStorage.getItem("theme_boost_union_scrollspy_closestid"),closestdelta=window.sessionStorage.getItem("theme_boost_union_scrollspy_closestdelta");if(closestid&&closestdelta){let y=document.getElementById(closestid).offsetTop+parseInt(closestdelta);document.getElementById("page").scrollTo(0,y)}window.sessionStorage.removeItem("theme_boost_union_scrollspy_edittoggled"),window.sessionStorage.removeItem("theme_boost_union_scrollspy_closestid"),window.sessionStorage.removeItem("theme_boost_union_scrollspy_closestdelta")}}};_exports.init=()=>{var callback;callback=initScrollSpy,"complete"===document.readyState||"interactive"===document.readyState?setTimeout(callback,1):document.addEventListener("DOMContentLoaded",callback)}}));
