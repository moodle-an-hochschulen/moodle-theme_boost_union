...
if ($hassiteconfig || has_capability('theme/boost_union:configure', context_system::instance())) {
    ...
    if (!$ADMIN->fulltree) {
        ...
        $setting = new theme_boost_union\admin_setting_configselect_with_courseoverride($name, $title, $description,
                THEME_BOOST_UNION_SETTING_COURSEHEADERTEXTONIMAGESTYLE_LIGHT, $courseheadertextonimagestyleoptions, false);
        $setting->set_updatedcallback('theme_boost_union_purge_courseoverrides_cache');
        $tab->add($setting);
        $page->hide_if('theme_boost_union/courseheadertextonimagestyle', 'theme_boost_union/courseheaderenabled', 'neq',
                THEME_BOOST_UNION_SETTING_SELECT_YES);
