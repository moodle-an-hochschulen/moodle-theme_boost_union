...
class course_settings_transfer {
     ...
     *
     * This method contains the shared logic for validating and saving course-specific
     * theme settings, used by both the restore class and the course restored event observer.
     *
    ...
    public static function transfer_course_settings(array $originalsettings, int $newcourseid): void {
        ...
        foreach ($originalsettings as $setting) {
            ...
            // If the setting is not enabled for course import transfer, skip it.
            ...
            if ($importtransferconfig === false || $importtransferconfig != THEME_BOOST_UNION_SETTING_SELECT_ALWAYS) {
                continue;
            }
    ...
    /**
     * Transfer course files from original course to new course during import.
     *
    ...
    public static function transfer_course_files(int $originalcourseid, int $newcourseid): void {
        ...
        foreach ($fileareas as $filearea => $config) {
            // If the file area is not enabled for course import transfer, skip it.
