...
/**
 * Recommendation for Moodle core favicon usage with Boost Union.
 *
 * @package    theme_boost_union
 * @copyright  2026 Alexander Bias <bias@alexanderbias.de>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class corefavicon implements recommendation {
    /** @var string|null */
    ...
    public function get_status(): string {
        ...
        // If a core favicon is configured or uploaded, show a notice. Otherwise this is fine.
        $favicon = (string) get_config('core_admin', 'favicon');
        if ($favicon === '') {
            $fs = get_file_storage();
            $systemcontext = \context_system::instance();
            if (!$fs->is_area_empty($systemcontext->id, 'core_admin', 'favicon', 0)) {
                $favicon = '1';
            }
        }
     ...
     *
     * Removes the core favicon config value and deletes all files in the favicon filearea.
     *
     * @return void
     */
    public function autofix(): void {
        // Remove config and files.
        unset_config('favicon', 'core_admin');
        $fs = get_file_storage();
        $systemcontext = \context_system::instance();
        $fs->delete_area_files($systemcontext->id, 'core_admin', 'favicon', 0);
