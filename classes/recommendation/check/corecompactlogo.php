...
/**
 * Core compact logo recommendation.
 *
...
/**
 * Recommendation for Moodle core compact logo usage with Boost Union.
 *
 * @package    theme_boost_union
 * @copyright  2026 Alexander Bias <bias@alexanderbias.de>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class corecompactlogo implements recommendation {
    /** @var string|null */
    ...
    public function get_title(): string {
        return get_string(
            ...
            'theme_boost_union',
            get_string('logocompactsetting', 'theme_boost_union')
        );
    }
    ...
    public function get_status(): string {
        ...
        // If a core compact logo is configured or uploaded, show a notice. Otherwise this is fine.
        $compactlogo = (string) get_config('core_admin', 'logocompact');
        if ($compactlogo === '') {
            $fs = get_file_storage();
            $systemcontext = \context_system::instance();
            if (!$fs->is_area_empty($systemcontext->id, 'core_admin', 'logocompact', 0)) {
                $compactlogo = '1';
            }
        }
     ...
     *
     * Removes the core compact logo config value and deletes all files in the logocompact filearea.
     *
     * @return void
     */
