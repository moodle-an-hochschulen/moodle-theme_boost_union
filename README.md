...
[![Moodle Plugin CI](https://github.com/moodle-an-hochschulen/moodle-theme_boost_union/actions/workflows/moodle-plugin-ci.yml/badge.svg?branch=MOODLE_502_STABLE)](https://github.com/moodle-an-hochschulen/moodle-theme_boost_union/actions?query=workflow%3A%22Moodle+Plugin+CI%22+branch%3AMOODLE_502_STABLE)
...
* Login page widgets:
  In addition to the previous item, to implement the various login page options, we sometimes had to diverge from the Moodle Core login page behaviour as it would have been too complicated to keep up exactly the same look as Moodle core for all Boost Union setting permutations. Against this background, you should be aware that the out-of-the box configuration of Boost Union might not 100% match the login page look of Moodle core and you might even not be able to always replicate 100% the same login page look as you would get with Moodle Core. On the other hand, with the Boost Union login page settings you are much more flexible.
* Footer popover:
...
* Activity and section navigation styling:
  Boost Union restyles the core activity navigation (previous / next activity links) and section navigation (previous / next section links) to look like Bootstrap small secondary buttons and, along the way, fixes the position of the "next section" arrow which Moodle core renders in front of the section name instead of behind it. This restyling is always active as soon as the "Activity & section navigation elements" setting is enabled and there is intentionally no setting to switch back to the original Moodle core look. This is simply because the original look is rather oldfashioned and Moodle core does not really show these navigation elements anywhere else in its default course formats anymore.
