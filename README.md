...
Click the badge below to open the `main` branch instantly in [Moodle Playground](https://moodle-playground.com) with the theme pre-installed and activated. The playground boots a full Moodle 5.2 site, with a demo course already created, so you can preview Boost Union rendering real Moodle content without any local setup. Every pull request also automatically generates a playground preview link appended to the PR description, so reviewers can test theme changes in a live Moodle instance.
