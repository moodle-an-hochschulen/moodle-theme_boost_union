...
function theme_boost_union_get_fontawesome_extensions() {
    return ['.css', '.eot', '.svg', '.ttf', '.woff', '.woff2'];
}
...
function theme_boost_union_get_fontawesome_filestructure($version) {
    ...
    switch ($version) {
        case THEME_BOOST_UNION_SETTING_FAVERSION_FA6FREE:
            $files = ['css' => ['fontawesome.min.css' => THEME_BOOST_UNION_SETTING_FAFILES_MANDATORY,
                    ...
                    'webfonts' => ['fa-solid-900.woff2' => THEME_BOOST_UNION_SETTING_FAFILES_MANDATORY,
        ...
        if ($filestructure != null) {
            ...
            foreach ($filestructure as $folder => $files) {
                ...
                foreach ($files as $file => $expected) {
                    ...
                    if ($cachedfolder == null || ($cachedfolder == []) && count($cachedfolder) < 1) {
                        $exists = false;
                    ...
                    $filesforcontext[] = ['filepath' => $filepath, 'exists' => $exists, 'mandatory' => $mandatory,
...
function theme_boost_union_get_fontawesome_checks_templatecontext() {
    ...
    switch ($version) {
        case THEME_BOOST_UNION_SETTING_FAVERSION_FA6FREE:
            $checks = [
                    ['icon' => '<i class="fa fa-check-circle-o fa-3x fa-fw"></i>',
                    ...
                    ['icon' => '<i class="fa fa-map-o fa-3x fa-fw"></i>',
                    ...
                    ['icon' => '<i class="fa-solid fa-virus-covid fa-3x fa-fw"></i>',
    ...
    if (file_exists($CFG->dirroot.'/filter/fontawesome/version.php')) {
        $checks[] = ['icon' => format_text('[fa-solid fa-users-line fa-3x fa-fw]'),
 ...
 *
 * @return string The legacy implementation will return a string.
 */
function theme_boost_union_callbackimpl_before_standard_html() {
    global $CFG;
