...
$string['loginshibbolethinternalwayfsetting'] = 'Internal Shibboleth WAYF';
$string['loginshibbolethinternalwayfsetting_desc'] = 'With this setting, you can replace a Shibboleth login button with an internal WAYF form. If set to "Yes (based on the \'auth_shibboleth\' configuration)", the Shibboleth login button is replaced by the same organisation (IdP) selector used on the <a href="{$a->loginurl}">Shibboleth authentication plugin\'s internal login page</a>. The list of organisations comes from the <a href="{$a->settingsurl}">Shibboleth authentication plugin\'s \'Identity providers\' configuration</a>. If that list is empty or Shibboleth authentication is not enabled at all, this setting does not have any effect and IDP buttons are shown as normal. If set to "Yes (based on JavaScript code)", the code set in loginidpexternalwayfcode is used, which embeddes a login element itself.';
$string['loginshibbolethinternalwayfsettinginternal'] = 'Yes (based on the \'auth_shibboleth\' configuration)';
$string['loginshibbolethinternalwayfsettingexternal'] = 'Yes (based on JavaScript code)';
// ... ... Setting: Embedded WAYF Javascript.
$string['embeddedshibbolethwayftextsetting'] = 'Embedded WAYF Javascript';
$string['embeddedshibbolethwayftextsetting_desc'] = 'With this setting, you can embed custom JavaScript code which renders a WAYF (Where Are You From) element directly on the login page without the need to link to an external WAYF page. This allows you to provide a seamless login experience for Shibboleth users without forcing them to leave the login page. Please note that you need to provide the complete JavaScript code including the necessary HTML elements and event handlers for the WAYF functionality. This setting only takes effect if the "Yes (based on JavaScript code)" option is selected in the "Internal Shibboleth WAYF" setting above. Your IDP will most likely pass you the needed code.';
