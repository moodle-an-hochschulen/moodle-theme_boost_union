...
class smartmenu_helper {
    ...
    public function restriction_byroles(&$query) {
        ...
        $contextsql = ($this->data->rolecontext == smartmenu::SYSTEMCONTEXT) ? ' AND contextid=:systemcontext ' : '';
    ...
    public static function theme_boost_union_menuitems_button(int $menuid) {
        ...
        // Add new heading.
        $itemscaption = get_string('smartmenu:addnewheadingitem', 'theme_boost_union');
        $itemsurl = new moodle_url(
            '/theme/boost_union/smartmenus/edit_items.php',
            ['menu' => $menuid, 'type' => smartmenu_item::TYPEHEADING, 'sesskey' => sesskey()]
        );
        $output .= html_writer::link($itemsurl, $itemscaption);
        // Add new item.
        $itemscaption = get_string('smartmenu:addnewstaticitem', 'theme_boost_union');
        $itemsurl = new moodle_url(
            '/theme/boost_union/smartmenus/edit_items.php',
            ['menu' => $menuid, 'type' => smartmenu_item::TYPESTATIC, 'sesskey' => sesskey()]
        );
        $output .= html_writer::link($itemsurl, $itemscaption);
        // Add new item.
        $itemscaption = get_string('smartmenu:addnewdynamicitem', 'theme_boost_union');
        $itemsurl = new moodle_url(
            '/theme/boost_union/smartmenus/edit_items.php',
            ['menu' => $menuid, 'type' => smartmenu_item::TYPEDYNAMIC, 'sesskey' => sesskey()]
        );
        $output .= html_writer::link($itemsurl, $itemscaption);
