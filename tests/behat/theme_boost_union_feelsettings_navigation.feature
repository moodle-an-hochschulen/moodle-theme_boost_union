...
Feature: Configuring the theme_boost_union plugin for the "Navigation" tab on the "Feel" page
  ...
  Scenario: Setting: Course category breadcrumbs (verify that course sections are properly displayed)
    Given the following "categories" exist:
      | name           | category | idnumber | category |
      | Category E     | 0        | CE       | 0        |
      | Category ED    | 1        | CED      | CE       |
    ...
    And "Topic 1" "link" should appear after "Category E" "link" in the ".breadcrumb" "css_element"
