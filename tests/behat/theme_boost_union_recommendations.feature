...
Feature: Recommendations in theme_boost_union
  ...
  Scenario Outline: Verify Recommendation corelogo / corecompactlogo / corefavicon (Status check for negative case)
    ...
    And I upload "<uploadfile>" file to "<filearea>" filemanager
    ...
    Examples:
      ...
      | Compact logo | theme/boost_union/tests/fixtures/login_bg2.png | Compact logo upload |
  ...
  Scenario: Verify recommendation notification and view-all action on settings page (with the core logo recommendation as an example)
    Given I log in as "admin"
    And I navigate to "Appearance > Logos" in site administration
    And I upload "theme/boost_union/tests/fixtures/login_bg1.png" file to "Logo" filemanager
    And I press "Save changes"
