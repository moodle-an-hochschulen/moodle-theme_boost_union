...
Feature: Configuring the theme_boost_union plugin for the "Course header" section on the "Course" tab on the "Look" page
  ...
  Scenario Outline: Setting: Course header - Define the course header canvas border (globally).
    ...
    Examples:
      ...
      | grey       | border border-secondary | should      |
      | brandcolor | border border-primary   | should      |
  ...
  Scenario Outline: Setting: Course header - Define the course header text on image color (globally).
    Given the following config values are set as admin:
      | config                        | value       | plugin            |
      | courseheaderenabled           | yes         | theme_boost_union |
      | courseheaderimagesource       | global      | theme_boost_union |
      | courseheaderlayout            | <layout>    | theme_boost_union |
      | courseheadertextonimagecolor  | <textcolor> | theme_boost_union |
    And the following "theme_boost_union > setting file" exists:
      | filearea | courseheaderimageglobal                        |
      | filepath | theme/boost_union/tests/fixtures/login_bg1.png |
    When I log in as "student1"
    ...
    # We do not want to burn too much CPU time by testing all possible options. We just test both available values and one layout where the text color should not be set.
    Examples:
      | layout       | textcolor | textclass  | shouldornot |
      | stacked      | white     | text-white | should      |
      | stacked      | black     | text-dark  | should      |
      | headingabove | white     | text-white | should not  |
      | headingabove | black     | text-dark  | should not  |

  Scenario: Setting: Course header - Define the course header text on image color (per course).
    Given the following config values are set as admin:
      | config                                       | value   | plugin            |
      | courseheaderenabled                          | yes     | theme_boost_union |
      | courseheaderimagesource                      | global  | theme_boost_union |
      | courseheaderlayout                           | stacked | theme_boost_union |
      | courseheadertextonimagecolor                 | white   | theme_boost_union |
      | courseheadertextonimagecolor_courseoverride  | 0       | theme_boost_union |
    And the following "theme_boost_union > setting file" exists:
      | filearea | courseheaderimageglobal                        |
      | filepath | theme/boost_union/tests/fixtures/login_bg1.png |
    When I log in as "teacher1"
    And I am on "Course 1" course homepage
    And I click on "Settings" "link"
    And I should not see "Course header text on image color"
    And the following config values are set as admin:
      | config                                      | value | plugin            |
      | courseheadertextonimagecolor_courseoverride | 1     | theme_boost_union |
    And I reload the page
    And I should see "Course header text on image color" in the "#id_theme_boost_union_course_courseheaderhdr" "css_element"
    And I set the field "Course header text on image color" to "Black"
    And I press "Save and display"
    And I log out
    And I log in as "student1"
    And I am on "Course 1" course homepage
    Then "#courseheaderimage" "css_element" should exist
    And the "class" attribute of ".bucoursetitle" "css_element" should contain "text-dark"
