...
Feature: Configuring the theme_boost_union plugin for the "Course" tab on the "Feel" page
  ...
  @javascript
  Scenario: Setting: Appearance of sections - The settings are hidden (and a notice is shown) when the course shows one section per page
    Given the following config values are set as admin:
      | config                                  | value | plugin            |
      | sectionzeroappearance_courseoverride    | 1     | theme_boost_union |
    ...
    And I click on "Settings" "link"
    And I expand all fieldsets
    # With the 'Show all sections on one page' course layout, the settings are shown and the notice is hidden.
    And I set the field "Course layout" to "Show all sections on one page"
    Then "Appearance of section 0" "field" should be visible
    And "Appearance of sections ≥ 1" "field" should be visible
    And I should not see "there is nothing to configure here"
    # With the 'Show one section per page' course layout, the settings are hidden and the notice is shown instead.
    And I set the field "Course layout" to "Show one section per page"
    Then "Appearance of section 0" "field" should not be visible
    And "Appearance of sections ≥ 1" "field" should not be visible
    And I should see "there is nothing to configure here"
    # The notice spans the full width of the form via the 'theme-boost-union-fullwidth-formelement' CSS utility class.
    And "#fitem_id_theme_boost_union_course_sectionsnosettings.theme-boost-union-fullwidth-formelement" "css_element" should exist
